...
def get_incoming_hormuud_messages_from_recovery_csv(csv_path,
    ...
    incoming_recovered_messages = [msg for msg in all_recovered_messages if msg["Destination"] == TARGET_SHORTCODE]
    ...
    for msg in incoming_recovered_messages:
        ...
        try:
            ...
            msg["received_at"] = pytz.timezone("Africa/Mogadishu").localize(
                datetime.strptime(msg["ReceivedAt"], "%Y/%m/%d %H:%M:%S")
            )
    ...
    if received_after_inclusive is not None:
        ...
        incoming_recovered_messages = [msg for msg in incoming_recovered_messages
                                       if msg["received_at"] >= received_after_inclusive]
    ...
    if received_before_exclusive is not None:
        log.info(f"Filtering out messages sent before {received_before_exclusive}...")
        incoming_recovered_messages = [msg for msg in incoming_recovered_messages
                                       if msg["received_at"] < received_before_exclusive]
        log.info(f"Filtered out messages sent before {received_before_exclusive}: "
                 f"{len(incoming_recovered_messages)} messages remain")
...
if __name__ == "__main__":
    ...
    recovered_messages.sort(key=lambda msg: msg["received_at"])
    ...
    for rapid_pro_msg in rapid_pro_messages:
        ...
        for recovery_item in recovered_lut[rapid_pro_msg.urn]:
            if recovery_item["rapid_pro_message"] is None and \
                    recovery_item["recovered_message"]["text"] == rapid_pro_text and \
                    rapid_pro_msg.sent_on - recovery_item["recovered_message"]["received_at"] < timedelta(minutes=5):
            ...
            for recovery_item in recovered_lut[rapid_pro_msg.urn]:
                if recovery_item["rapid_pro_message"] is None and \
                    ...
                    writer.writerow({
                        ...
                        "Hormuud Recovery": recovery_item["recovered_message"]["text"]
                    })
    ...
    with open(output_csv_path, "w") as f:
        ...
        for msg in unmatched_recovered_messages:
            writer.writerow({
                "Sender": msg["urn"].replace("tel:+", ""),
                "Receiver": msg["Destination"],
                "Message": msg["text"],
                "ReceivedOn": msg["ReceivedAt"]
            })
